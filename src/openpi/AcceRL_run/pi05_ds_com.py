...
def _named_tensors_in_order(module, prefix=None, include_buffers=False):
    """
    只返回需要广播的张量：
      - 可训练参数 (requires_grad=True)
      - 可选的 buffers (pi0.5 第一版默认不广播；模型没有训练态 BN 统计量)
    ...
    params = sorted([(n, p) for n, p in filtered_params.items() if p.requires_grad], key=lambda x: x[0])
    buffers = sorted(list(filtered_buffers.items()), key=lambda x: x[0]) if include_buffers else []
    names = [name for name, _ in params] + [name for name, _ in buffers]
    if len(names) != len(set(names)):
        raise RuntimeError("广播张量名称不唯一，无法保证 Trainer/Inference 参数顺序")
...
def init_collective_group(
    ...
    global _group_mgr
    if not group_name:
        raise ValueError(f"group_name '{group_name}' 必须是一个非空字符串。")
...
def _broadcast_sanity_check_impl(role, group_name, dtype_name="float32", numel=8, src_rank=0):
    ...
    print(
        f"[broadcast_sanity][after] rank={rank} group={group_name} dtype={tensor.dtype} first={first} ok={ok}",
        flush=True,
    )
    if not ok:
...
class TrainerActorCom:
    ...
    def broadcast_weights(self, group_name, prefix=None):
        ...
        params, buffers = _named_tensors_in_order(module, prefix=prefix, include_buffers=False)
        if not params:
            raise RuntimeError("没有找到可训练的 pi0.5 参数，拒绝执行空权重广播")
    ...
    def get_broadcast_signature(self):
        ...
        params, buffers = _named_tensors_in_order(module, include_buffers=False)
        return [("param", name, tuple(tensor.shape), str(tensor.dtype)) for name, tensor in params] + [
            ("buffer", name, tuple(tensor.shape), str(tensor.dtype)) for name, tensor in buffers
        ]
...
class InferenceActorCom:
    ...
    def receive_and_update_weights(self, group_name):
        ...
        params, buffers = _named_tensors_in_order(module, include_buffers=False)
        if not params:
            raise RuntimeError("没有找到可训练的 pi0.5 参数，拒绝执行空权重接收")
        device = next(module.parameters()).device
